    ...
    <Container size="100%" mb="50px" pos="relative">
      <Stack mb="md">
        <Group justify="space-between">
          ...
          <Button onClick={() => setRegister(true)}>Adicionar Unidade</Button>
        </Group>
        <Grid>
          {permissionsData?.sa && (
                  ...
                  <Table.Tr key={row.id}>
                    ...
                    <Table.Td className={classes.td}>
                      <Group gap="xs" justify="flex-end">
                        <Button size="compact-sm" component={Link} color="orange" title="Editar" href={`/unidades/${row.id}`}>Editar</Button>
                      </Group>
                    </Table.Td>
                  </Table.Tr>
                )
      ...
      <Modal opened={register} onClose={() => setRegister(false)} title="Cadastrar unidade" centered>
        <FormCompany.Basic onClose={() => {
          setRegister(false)
          mutate()
        }} />
      </Modal>
...
export default guardAccount(Companies)
