...
function Categories() {
  ...
  const { data, error, mutate } = useFetch([
    isAuthenticated ? '/admin/service-categories' : null,
    ...
    <Container size="100%" mb="50px">
      <Stack mb="md">
        <Group justify="space-between">
          ...
          <Button onClick={() => setRegister(true)}>Adicionar Categoria</Button>
        </Group>
        <Grid>
          {permissionsData?.sa && (
                  ...
                  <Table.Tr key={row.id}>
                    ...
                    <Table.Td className={classes.td}>
                      <Group gap="xs" justify="flex-end">
                        <Button size="compact-sm" component={Link} color="orange" title="Editar" href={`/categorias/${row.id}`}>Editar</Button>
                      </Group>
                    </Table.Td>
                  </Table.Tr>
                )
...
export default guardAccount(Categories)
